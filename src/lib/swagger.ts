...
export const swaggerSpec = {
  ...
  paths: {
            ...
            "application/json": {
              schema: {
                ...
                properties: {
                  ...
                  subscriber_photo_url: { type: "string", description: "Legacy: pre-hosted URL. Ignored if subscriber_photo is also provided." },
                  passport_bio_page_url: { type: "string", description: "Legacy: pre-hosted URL. Ignored if passport_bio_page is also provided." },
                  visa_page_url: { type: "string", description: "Legacy: pre-hosted URL. Ignored if visa_page is also provided." },
                  application_form_url: { type: "string", description: "Legacy: pre-hosted URL. Ignored if application_form is also provided." },
                  subscriber_photo: { type: "string", format: "byte", description: "Preferred. Inline image as a base64 data URI, e.g. data:image/jpeg;base64,<data>. Allowed types: jpeg, png, gif, webp, bmp, tiff, heic, heif. Max 10MB decoded." },
                  passport_bio_page: { type: "string", format: "byte", description: "Preferred. Inline image as a base64 data URI. Same format/limits as subscriber_photo." },
                  visa_page: { type: "string", format: "byte", description: "Preferred. Inline image as a base64 data URI. Same format/limits as subscriber_photo." },
                  application_form: { type: "string", format: "byte", description: "Preferred. Inline image or PDF as a base64 data URI. Same image types as subscriber_photo, plus application/pdf. Max 10MB decoded." },
                },
              },
            },
          },
    ...
    "/documents/subscribers/{id}/{type}/raw": {
      get: {
        tags: ["Documents"],
        summary: "Fetch raw document bytes (image or PDF)",
        description: "Serves the document body directly (Content-Type set to the stored MIME type) — either bytes stored in the database from an inline POST /subscribers upload, or a 302 redirect to the legacy hosted URL (Supabase/local) for older records. Not authenticated, since the dashboard loads this as a plain <img src>.",
        parameters: [
        ...
        responses: {
          "200": { description: "Document bytes", content: { "application/octet-stream": { schema: { type: "string", format: "binary" } } } },
          "302": { description: "Redirect to legacy hosted URL" },
          "400": { description: "Invalid document type", content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } },
